...
def get_wikipedia_info(driver_name):
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            search_response = requests.get(search_api, params=params, timeout=10)
        ...
        print(f"Erro ao buscar informações para {driver_name}: {e}")
        return None, None

# Carregar o JSON atual
print("Carregando JSON...")
with open('C:\\Users\\GCO\\CascadeProjects\\JSON\\formula1_drivers_updated_v2.json', 'r', encoding='utf-8-sig') as f:
    drivers = json.load(f)
...
for driver in drivers:
    ...
    # Verificar se já tem as datas de nascimento e morte
    has_birth = "BirthDate" in driver and driver["BirthDate"] and not driver["BirthDate"].startswith("94 and") and not driver["BirthDate"].startswith("55 and")
    has_death = "DeathDate" in driver
    
    if has_birth and has_death:
        print(f"[{processed}/{total_drivers}] {driver_name} já possui datas completas. Pulando...")
        continue
    ...
    if not has_birth:
        ...
        if processed % 5 == 0 or updated % 5 == 0:
            print(f"Salvando progresso... ({processed}/{total_drivers}, {updated} atualizações)")
            with open('C:\\Users\\GCO\\CascadeProjects\\JSON\\formula1_drivers_updated_v3.json', 'w', encoding='utf-8') as f:
                json.dump(drivers, f, indent=4, ensure_ascii=False)
            
            # Atualizar também o arquivo usado pelo site
            with open('C:\\Users\\GCO\\CascadeProjects\\F1Project\\data\\drivers.json', 'w', encoding='utf-8') as f:
                json.dump(drivers, f, indent=4, ensure_ascii=False)
...
print(f"Concluído! Processados {total_drivers} pilotos, {updated} atualizações.")
